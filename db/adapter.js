...
const DB_TYPE = (process.env.DB_TYPE || process.env.DB || 'sqlite').toLowerCase();
...
if(DB_TYPE === 'pg'){
  ...
  module.exports = {
    ...
    async run(sql, params=[]){
      sql = this._convert(sql, params);
      // append RETURNING id for inserts when absent
    ...
    async all(sql, params=[]){ sql = this._convert(sql, params); const r = await pool.query(sql, params); return r.rows; },
    async get(sql, params=[]){ sql = this._convert(sql, params); const r = await pool.query(sql, params); return r.rows[0]; },
    async init(){
  ...
  module.exports = {
    ...
    run, all, get,
    async init(){
      await run(`CREATE TABLE IF NOT EXISTS vehicles (id INTEGER PRIMARY KEY AUTOINCREMENT, num TEXT UNIQUE, driver TEXT, type TEXT, phone TEXT)`);
      await run(`CREATE TABLE IF NOT EXISTS trips (id INTEGER PRIMARY KEY AUTOINCREMENT, vehicleId INTEGER, vehicleNum TEXT, driver TEXT, date TEXT, material TEXT, cft INTEGER, rate REAL, amount REAL, loadTime TEXT, dumpTime TEXT, source TEXT, dest TEXT, note TEXT)`);
      const r = await get('SELECT COUNT(*) as c FROM vehicles');
      if(r && r.c === 0){
        const seed = [
          ['BA 12 PA 1234','Ram Bahadur','Tipper','9841001234'],
          ['GA 1 JA 5678','Hari Prasad','Truck','9851005678'],
        ...
        for(const s of seed) await run('INSERT INTO vehicles(num,driver,type,phone) VALUES(?,?,?,?)', s);
      }
      const t = await get('SELECT COUNT(*) as c FROM trips');
      if(t && t.c === 0){
        const mats = ['Sand','Rock','Aggregate','Miscuts'];
        const rates = {Sand:180,Rock:220,Aggregate:200,Miscuts:150};
        const srcs = ['Crusher A','Crusher B','Quarry 1'];
        ...
        for(let i=0;i<30;i++){
          ...
          const drv = vnum==='BA 12 PA 1234' ? 'Ram Bahadur' : vnum==='GA 1 JA 5678' ? 'Hari Prasad' : vnum==='BA 3 CHA 9999' ? 'Sita Devi' : 'Bikram Thapa';
          const d = new Date(); d.setDate(d.getDate()-Math.floor(Math.random()*20));
          await run('INSERT INTO trips(vehicleId,vehicleNum,driver,date,material,cft,rate,amount,loadTime,dumpTime,source,dest,note) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)', [1,vnum,drv,d.toISOString().split('T')[0],mat,cft,rate,cft*rate,'07:30','08:30',srcs[Math.floor(Math.random()*srcs.length)],dsts[Math.floor(Math.random()*dsts.length)],'']);
        }
      }
    }
  };
